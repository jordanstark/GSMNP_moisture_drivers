...
#### setup ####
  ...
  # paths
    ...
    climate_path <- paste0(gis_path,"Microclimate/micromod/") 
    
    out_min <- paste0(gis_path,"Microclimate/tmpMin/")
    if(!dir.exists(out_min)) dir.create(out_min)
    
    out_max <- paste0(gis_path,"Microclimate/tmpMax/")
    if(!dir.exists(out_max)) dir.create(out_max)
...
## run calculation in parallel and save files
    CalcTMP <- function(doy){
      ...
      writeRaster(tmp_min,
                  paste(out_min,"d_",sprintf("%03d",doy),sep=""),
                  format = "GTiff")
      ...
      writeRaster(tmp_max,
                  paste(out_max,"d_",sprintf("%03d",doy),sep=""),
                  format="GTiff")
