...
#### Setup ####
  ...
  ## import data
    ...
    # scaled summer data
    scaledat <- read.csv(paste0(intermediate_path,"scaled_summer_vmc_drivers.csv"))
    ...
    load(paste0(model_out_path,"summer_drivers_lmer.RData"))
...
#### figure of effect of drivers on summer moisture ####
    vars <- c("elev","log_tci","slope","tpi","prec","vpd","rad","meant","maxEVI")
    labels <- c("elevation","log(TCI)","slope","TPI","precipitation","VPD","radiation","below-canopy temperature","EVI")
    ...
    for(i in 1:length(vars)){
      ...
      plots[[i]] <- ggplot(fig.df, aes_string(x=var,y="vmc")) +
        geom_point(data=scaledat,aes(color=SiteID),alpha=0.1) +
        geom_line() +
        theme_bw() + 
